...
- **Zod is a runtime dependency too, and the wire gate is blind to it.** It renders every tool's advertised JSON Schema, so a Zod release can reshape what clients see with `tsc` green — the same exposure the SDK has. But `mcp-wire.test.ts` cannot see this half: it compares what the SDK advertises against `test/tool-schema.ts`'s derivation, and both call the same `z.toJSONSchema`, so when Zod changes its answer both sides move together and the equality still holds. `packages/mcp/test/json-schema-generation.test.ts` is the gate for that — it pins how each construct renders against expectations written by hand, with two coverage tests derived from `ALL_TOOL_SPECS` so a tool reaching for an unpinned construct fails until a canary covers it. Zod 4.5 is what proved the gap: a union whose members carry nothing but a `type` began rendering as `type: [a, b]` instead of `anyOf`, reshaping ten tools' advertised schemas with every gate green.
