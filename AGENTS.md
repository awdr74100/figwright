...
- **The MCP SDK is a runtime dependency, so `tsc` is not a gate for it.** `@modelcontextprotocol/server` (v2) generates every tool's advertised JSON Schema and negotiates the protocol version at runtime; an SDK release can change what clients see while typecheck stays green. Two e2e tests are the gate, both spawning the built `dist` over real stdio: `mcp-wire.test.ts` asserts the advertised contract by speaking raw JSON-RPC, and `client-compat.test.ts` drives the same surface through the **v1** client library that most MCP clients still embed. Like the other `dist`-backed e2e tests they **skip** when `dist` is missing, so a local `pnpm test` without a build silently proves nothing; CI always builds first.
