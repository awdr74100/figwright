...
4. Group dependent writes. Use one batch call for invertible writes that belong to the same build. Give a create operation an as alias, then use {"$ref": "alias.nodeId"} in later operations to style or nest the created node. The plugin keeps the operations ordered and rolls them back together if one fails. For a static mockup where editability is not required, prefer one import_svg per screen or section.

5. Text. create_text / set_text. A new TEXT node defaults to a fallback font (Inter), NOT the design system's font — set the real family/style with set_text_properties (the font must be available; the plugin loads the node's fonts on edit).

6. Reference tokens, not literals. Colour (fill / stroke): set the paint first (set_fills / set_strokes), then bind_variable_to_paint (target fills|strokes, index, variableId) — NOT bind_variable_to_node, which rejects fills/strokes because Figma stores colour bindings on the paint. Scalars (width / height / padding* / itemSpacing, per-corner radius topLeftRadius…, characters): bind_variable_to_node (field, variableId). Shared styles: apply_style_to_node (field fill / stroke / effect / grid / text). Never hardcode hex/px when a token exists — get_variable_defs tells you what does.

7. Verify visually (close the loop). After each section, get_screenshot the built node and compare it to the source intent; trace each discrepancy, fix, and re-screenshot. An empty:true export means the node rendered nothing (hidden / off-canvas).
