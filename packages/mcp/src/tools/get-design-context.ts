...
export const getDesignContextTool: ToolSpec = {
  ...
  description:
    ...
    'leading note saying which shape you got: first to the same tree with LAYOUT intact and only ' +
    'appearance dropped (every frame keeps its flex/grid mode, padding, gap and alignment, each ' +
    'node its sizing/constraints, each text its characters — enough to build correct containers, ' +
    'then re-ground per section for colour and type), then to the compact structure of the same ' +
    "tree, then to a sectionPlan. Whatever shape comes back, build layout from each frame's own " +
    'layout object — never reconstruct spacing from child x/y as margins or absolute offsets. ' +
