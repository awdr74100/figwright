...
Emit code in the detected stack (the profile is returned on component_map / token_map). Rules: reuse beats regenerate, reference tokens not literals, export visual assets rather than faking them with grey boxes or hand-typed wordmarks, output responsive code with a w-full root (never the artboard's fixed width), and mirror the project's existing import style, file layout, and naming. For an edge-to-edge (full-bleed) page, the body margin must be zeroed (a missing reset shows as a full-page white gutter + horizontal overflow, since scoped / CSS-module styles can't reach html/body) — but check first, don't blindly add one: Tailwind's preflight (profile.styling.system 'tailwind') and any existing reset/normalize already do this, so only add a minimal global reset (body { margin: 0 } + box-sizing) when the project is non-Tailwind and has none.`;
};
