...
const isExportable = (node: BaseNode): node is BaseNode & ExportMixin =>
  'exportAsync' in node;
...
export const createGetScreenshotHandler =
  ...
  async params => {
    ...
    if (!Array.isArray(p.nodeIds) || p.nodeIds.length === 0 || p.nodeIds.some(id => typeof id !== 'string')) {
      throw new TypeError('get_screenshot: nodeIds must be a non-empty string[]');
    }
    if (p.format !== undefined && !isFormat(p.format)) {
      throw new TypeError(`get_screenshot: format must be one of ${SCREENSHOT_FORMATS.join(' / ')}`);
    }
    ...
    const settings: ExportSettings =
      format === 'SVG' ? { format: 'SVG' } : { format, constraint: { type: 'SCALE', value: scale } };
